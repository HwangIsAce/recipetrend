...
class CustomTrainer(Trainer):
    ...
    def evaluate(
        ...
        import numpy as np
        import random
        predictions = self.predict(random.sample(eval_dataset['input_ids'],10))
        # print(predictions.predictions.shape, predictions.label_ids.shape)

        ans = predictions.label_ids.ravel()
        pred = np.argmax(predictions.predictions[0], axis=2).ravel()
        
        accuracy_for_wandb = len(np.where((ans != -100) & (ans == pred))[0]) / len(np.where(ans != -100)[0])

        output.metrics['eval_acc'] = accuracy_for_wandb
